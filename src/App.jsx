    ...
    <div style={{background:HW.surface2,borderRadius:12,padding:16,marginBottom:12}}>
      ...
      {okr.due_date&&(
        <div style={{fontSize:11,color:HW.muted,marginTop:6}}>
          Due: {new Date(okr.due_date).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"})}
        </div>
      )}
      {onUpdate&&(
        <div style={{display:"flex",gap:8,marginTop:10,alignItems:"center"}}>
          <input type="number" defaultValue={okr.current}
            style={{background:HW.surface,border:`1px solid ${HW.border}`,color:HW.text,
              borderRadius:6,padding:"4px 8px",width:80,fontSize:13}}
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",
      ...
      <div style={{background:HW.surface,border:`2px solid ${HW.red}`,
        ...
        <button onClick={onDismiss} style={{background:HW.red,color:HW.white,border:"none",
          borderRadius:8,padding:"12px 32px",fontWeight:700,fontSize:14,cursor:"pointer",width:"100%"}}>
...
export default function App() {
  ...
  const eventIcons = {
    ...
    mentor_changed:"👤",dept_changed:"🏢",reactivated:"✅",note:"📝",inactive:"⏸"
  };
  ...
  async function handleSession(authUser) {
    ...
    if(data){
      if(data.role==="management"){setView("mgmt");fetchTrainees();fetchOkrs();fetchAllData();}
      else{
        setView("trainee");
        if(data.trainee_id){
  ...
  async function submitAttendance(){
    ...
    if(!profile.full_name||!profile.civil_id){setMsg("Please fill your profile first.");return;}
    ...
    const penaltyApplied=!attendance.attended||
      (attendance.signin_time&&attendance.signin_time>MAX_SIGNIN&&!isLate);
    setLoading(true);setMsg("");
    ...
    if(excuseType==="traffic"&&excusePhotoUrl){
      await supabase.from("traffic_excuses").insert({trainee_id:tid,report_date:attendance.report_date,photo_url:excusePhotoUrl});
    ...
    await supabase.from("daily_reports").upsert({
      trainee_id:tid,report_date:attendance.report_date,
      attended:attendance.attended,signin_time:attendance.signin_time||null,
  ...
  async function submitTasks(){
    ...
    if(!profileData?.trainee_id){setMsg("Please submit attendance first.");setLoading(false);setAiLoading(false);return;}
    ...
    let ai=null;
    try{ai=await analyzeReport(taskReport.report_text);setAiResult(ai);}catch(e){console.error(e);}
    await supabase.from("daily_reports").upsert({
  ...
  async function changeStatus(newStatus){
    const messages={
      dropped:"Trainee has left/dropped the program",inactive:"Trainee marked as inactive",
      transferred:"Trainee transferred to another department",active:"Trainee reactivated in the program",
    };
  ...
  async function exportPDF(trainee){
    ...
    autoTable(doc,{startY:56,
      ...
      body:[["Full Name",t.full_name||"—"],["Civil ID",t.civil_id||"—"],
        ...
        ["Days in Program",t.joining_date?Math.floor((new Date()-new Date(t.joining_date))/(1000*60*60*24))+" days":"—"]],
      headStyles:{fillColor:[207,10,44],textColor:[255,255,255]},
      alternateRowStyles:{fillColor:[245,245,245]},
    });
    const reportsY=doc.lastAutoTable.finalY+10;
    doc.setFontSize(14);doc.setFont("helvetica","bold");doc.text("Weekly Reports",14,reportsY);
    const{data:reps}=await supabase.from("daily_reports").select("*")
    ...
    autoTable(doc,{startY:penY+4,
      ...
      body:pens?.length>0?pens.map(p=>[p.report_date,p.reason,`-${p.amount}%`]):[["—","No penalties","—"]],
      headStyles:{fillColor:[207,10,44],textColor:[255,255,255]},
      alternateRowStyles:{fillColor:[245,245,245]},
    });
    doc.setTextColor(207,10,44);doc.setFontSize(11);doc.setFont("helvetica","bold");
    doc.text(`Total Deduction: ${pens?(pens.length*PENALTY_PCT).toFixed(2):0}%`,14,doc.lastAutoTable.finalY+6);
    ...
    <div style={{...s.page,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{...s.card,width:420}}>
        ...
        <input style={{...s.input,marginBottom:14}} type="password"
          value={password} onChange={e=>setPassword(e.target.value)} placeholder="Minimum 6 characters"/>
        <label style={s.label}>Confirm Password</label>
        <input style={{...s.input,marginBottom:20}} type="password"
          value={confirmPwd} onChange={e=>setConfirmPwd(e.target.value)} placeholder="Repeat your password"
        ...
        <button style={{...s.btn,background:"none",color:HW.muted,width:"100%",padding:13,marginTop:8}}
      ...
      {traineeTab==="attendance"&&(
        <div>
          ...
          <div style={{...s.card,border:faceCaptured?`1px solid rgba(207,10,44,.5)`:`1px solid ${HW.border}`}}>
            <h3 style={{marginBottom:12,color:HW.red}}>🤳 Face ID Verification</h3>
            ...
            {faceCaptured&&(
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                <div style={{width:56,height:56,borderRadius:"50%",background:"rgba(207,10,44,.15)",
          ...
          <div style={s.card}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              ...
              <div><label style={s.label}>Sign-in Time</label>
                <input style={s.input} type="time" value={attendance.signin_time}
                  onChange={e=>{setAttendance({...attendance,signin_time:e.target.value});setIsLate(e.target.value>MAX_SIGNIN);}}/></div>
              <div style={{display:"flex",alignItems:"center",gap:10,paddingTop:22}}>
                ...
                <label htmlFor="att" style={{...s.label,margin:0}}>I confirm attendance today</label>
              </div>
                ...
                <div>
                  ...
                  {excuseType==="traffic"&&(
                    <p style={{fontSize:11,color:HW.muted,marginTop:6}}>
                      Traffic excuses this month: <b style={{color:trafficCount>=2?HW.red:HW.text}}>{trafficCount}/2</b>
                    </p>
                  )}
                </div>
              </div>
        ...
        <div>
          <div style={s.card}>
            ...
            <p style={{color:HW.muted,fontSize:13,marginBottom:20}}>
              Describe everything you worked on today. The AI will analyze your report and generate a KPI score.
            </p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,paddingTop:22}}>
                ...
                <span style={{fontSize:12,color:HW.muted}}>Be detailed for a better AI score!</span>
              </div>
            ...
            <button style={{...s.btn,background:HW.red,color:HW.white,
              marginTop:16,width:"100%",padding:14,fontSize:15,opacity:(loading||aiLoading)?0.6:1}}
  ...
  if(view==="mgmt") {
    ...
    return (
      <div style={s.page}>
        ...
        <div style={{display:"flex",gap:4,background:HW.surface2,borderRadius:10,padding:4,marginBottom:24}}>
        ...
        {mgmtTab==="trainees"&&!selected&&(
          <div style={s.card}>
            <h3 style={{marginBottom:16}}>All Trainees
              <span style={{fontSize:13,color:HW.muted,fontWeight:400,marginLeft:8}}>— click a row to view profile</span>
            </h3>
          ...
          <div>
            ...
            <div style={{...s.card,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:16}}>
              <div style={{display:"flex",alignItems:"center",gap:16}}>
                ...
                <div>
                  ...
                  <div style={{display:"flex",gap:16,marginTop:4,flexWrap:"wrap"}}>
                    <span style={{fontSize:12,color:HW.muted}}>📅 Joined: <b style={{color:HW.text}}>{selected.joining_date||"—"}</b></span>
                    {selected.quitting_date&&<span style={{fontSize:12,color:HW.muted}}>🚪 Quit: <b style={{color:HW.red}}>{selected.quitting_date}</b></span>}
                    {selected.joining_date&&<span style={{fontSize:12,color:HW.muted}}>⏱ <b style={{color:HW.text}}>{Math.floor((new Date(selected.quitting_date||new Date())-new Date(selected.joining_date))/(1000*60*60*24))} days</b></span>}
                  </div>
                  <span style={{padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:700,marginTop:6,display:"inline-block",
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                <button style={{...s.btn,background:"rgba(207,10,44,.15)",color:HW.red,fontSize:12}} onClick={()=>exportPDF()}>📄 PDF</button>
                {selected.status!=="active"&&<button style={{...s.btn,background:"rgba(207,10,44,.15)",color:HW.red,fontSize:12}} onClick={()=>changeStatus("active")}>✅ Reactivate</button>}
                {selected.status!=="transferred"&&<button style={{...s.btn,background:"rgba(255,165,0,.15)",color:"#FFA500",fontSize:12}} onClick={()=>changeStatus("transferred")}>🔄 Transferred</button>}
                {selected.status!=="inactive"&&<button style={{...s.btn,background:"rgba(136,136,136,.15)",color:HW.muted,fontSize:12}} onClick={()=>changeStatus("inactive")}>⏸ Inactive</button>}
                {selected.status!=="dropped"&&<button style={{...s.btn,background:"rgba(100,100,100,.2)",color:"#666",fontSize:12}} onClick={()=>changeStatus("dropped")}>🔴 Dropped</button>}
              </div>
            </div>
                ...
                {logs.length===0?<p style={{color:HW.muted}}>No activity logged yet.</p>
                  :logs.map((log,i)=>(
                    <div key={log.id} style={{display:"flex",gap:16,marginBottom:20,position:"relative"}}>
                      {i<logs.length-1&&<div style={{position:"absolute",left:19,top:40,width:2,height:"calc(100% + 4px)",background:HW.border}}/>}
                    ...
                    return (
                      <div key={r.id} style={{background:HW.surface2,borderRadius:12,padding:16,marginBottom:14}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                          ...
                          <div style={{display:"flex",alignItems:"center",gap:12}}>
                            {r.penalty_applied&&<span style={{fontSize:11,color:HW.red,background:"rgba(207,10,44,.1)",padding:"2px 8px",borderRadius:10,fontWeight:700}}>⚠️ -{r.penalty_amount}%</span>}
                            {r.kpi_score&&<div style={{fontSize:22,fontWeight:800,color:kpiColor(r.kpi_score)}}>{r.kpi_score}<span style={{fontSize:11,color:HW.muted,fontWeight:400}}> KPI</span></div>}
                          </div>
                        </div>
                        <div style={{fontSize:11,color:HW.muted,marginBottom:8}}>
                          {r.signin_time||"no sign-in"} · <span style={{color:r.attended?"#34d399":HW.red}}>{r.attended?"● Attended":"○ Absent"}</span>
                        </div>
                        {r.excuse_type&&(
                          <div style={{background:"rgba(255,165,0,.08)",borderRadius:8,padding:10,marginBottom:10,borderLeft:"3px solid #FFA500"}}>
                        ...
                        {r.face_capture_url&&(
                          <div style={{marginBottom:10}}>
                            ...
                            <img src={r.face_capture_url} alt="face" style={{width:80,height:80,objectFit:"cover",borderRadius:"50%",border:`2px solid ${HW.red}`}}/>
                          </div>
                        )}
                        {pie&&(
            ...
            {profileTab==="penalties"&&(
              <div style={s.card}>
                ...
                {penalties.length===0?<p style={{color:HW.muted}}>No penalties recorded.</p>:(
                  <>
                    <div style={{background:"rgba(207,10,44,.08)",border:`1px solid rgba(207,10,44,.2)`,borderRadius:10,padding:14,marginBottom:16}}>
          ...
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:20}}>
              <div style={s.card}>
                ...
                {analytics.traineeKpi.length===0?<p style={{color:HW.muted}}>No data yet.</p>
                  :analytics.traineeKpi.slice(0,8).map((t,i)=>(
                    <div key={t.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:`1px solid ${HW.border}`}}>
                      <div style={{width:28,height:28,borderRadius:"50%",
                        background:i===0?"#FFD700":i===1?"#C0C0C0":i===2?"#CD7F32":HW.surface2,
                        display:"flex",alignItems:"center",justifyContent:"center",
                        fontSize:12,fontWeight:800,color:i<3?HW.dark:HW.muted,flexShrink:0}}>{i+1}</div>
                      <div style={{flex:1}}>
              ...
              <div style={s.card}>
                ...
                {analytics.deptData.length===0?<p style={{color:HW.muted}}>No data yet.</p>:<BarChart data={analytics.deptData}/>}
                <div style={{marginTop:16}}>
                  {analytics.deptData.map((d,i)=>(
                    <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${HW.border}`}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        ...
                        <span style={{fontSize:13,fontWeight:700,color:kpiColor(d.value),minWidth:30}}>{d.value}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{...s.card,border:`1px solid rgba(248,113,113,.3)`}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:12}}>
                  {analytics.atRisk.map(t=>(
                    <div key={t.id} style={{background:HW.surface2,borderRadius:12,padding:16,borderLeft:"3px solid #f87171"}}>
                      ...
                      <div style={{display:"flex",gap:12}}>
                        <div style={{textAlign:"center"}}><div style={{fontSize:20,fontWeight:800,color:"#f87171"}}>{t.avgKpi}</div><div style={{fontSize:10,color:HW.muted}}>Avg KPI</div></div>
                        <div style={{textAlign:"center"}}><div style={{fontSize:20,fontWeight:800,color:"#f87171"}}>{t.penalties}</div><div style={{fontSize:10,color:HW.muted}}>Penalties</div></div>
                        <div style={{textAlign:"center"}}><div style={{fontSize:20,fontWeight:800,color:HW.muted}}>{t.reports}</div><div style={{fontSize:10,color:HW.muted}}>Reports</div></div>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
              <div>
                ...
                <p style={{color:HW.muted,fontSize:13,margin:"4px 0 0"}}>Objectives and Key Results by Department</p>
              </div>
              <button style={{...s.btn,background:HW.red,color:HW.white}} onClick={()=>setShowAddOkr(!showAddOkr)}>
              ...
              <div style={{...s.card,border:`1px solid rgba(207,10,44,.3)`}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                  ...
                  <div><label style={s.label}>Unit</label>
                    <select style={s.input} value={newOkr.unit} onChange={e=>setNewOkr({...newOkr,unit:e.target.value})}>
                ...
                <button style={{...s.btn,background:HW.red,color:HW.white,marginTop:16}} onClick={addOkr}>Add OKR</button>
                {msg&&<p style={{color:msg.startsWith("✅")?"#34d399":HW.red,fontSize:13,marginTop:10}}>{msg}</p>}
              </div>
            )}
